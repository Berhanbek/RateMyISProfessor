...
import { secondYearProfessors } from '../data/professors';
import { ProfessorCard } from './ProfessorCard';
import { RatingModal } from './RatingModal';
import { fetchProfessorRatings } from '../services/ratingsService';

interface SecondYearRatingProps {
  onBack: () => void;
}

export const SecondYearRating: React.FC<SecondYearRatingProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    const loadAllRatings = async () => {
      const ratingsPromises = secondYearProfessors.map(async (professor) => {
        ...
        <div className="mb-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 dark:text-gray-300">
              Select a professor to share your experience
            </p>
          </div>
        </div>

        <div className="max-w-2xl mx-auto space-y-4">
          {secondYearProfessors.map((professor) => (
            <ProfessorCard
              key={professor.id}
              professor={professor}
